...
if(isset($_POST['submit'])){
  ...
  if ($valid){
    $validate="SELECT * FROM appointments WHERE appt_date = '$appt_date'";
    $result=mysqli_query($link, $validate);
    if (mysqli_num_rows($result)> 0) {
      while($row = mysqli_fetch_array($result)) {
          if ($row['appt_time'] == '$appt_time'){
            echo 'hi';
          }
      }
    }
  ...
  // $message = "We would like to inform you that we have recieved your appointment on ". $appt_date ." ". $appt_time ." for ". $services ."";
  // $headers = "Thank You, ". $_SESSION['username'] ."!";
...
<div class='pagebody clearfix'>
  <div class='content-container'>
    ...
    <div class='pagecontent clearfix'>
      ...
      else { echo "<div class='alert alert-danger'><strong>Sorry!</strong> Please login to view this page.</div>";}
      ?>
    </div>
  </div>
</div>
